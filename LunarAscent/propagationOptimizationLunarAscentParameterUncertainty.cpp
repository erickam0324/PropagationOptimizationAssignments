...
int main( )
{
    ...
    for( int i = 0; i < 100; i++ )
    {
        ...
        std::shared_ptr< SphericalHarmonicsGravityFieldSettings > EarthSphericalHarmonicGravityFieldSettings =
                        std::dynamic_pointer_cast< SphericalHarmonicsGravityFieldSettings >( bodySettings.at( "Earth" )->gravityFieldSettings );
                double nominalGravitationalParameter = EarthSphericalHarmonicGravityFieldSettings->getGravitationalParameter( );

                double SunnominalGravitationalParameter = spice_interface::getBodyGravitationalParameter( "Sun" );



               // std::shared_ptr< SphericalHarmonicsGravityFieldSettings > SunSphericalHarmonicGravityFieldSettings =
                 //               std::dynamic_pointer_cast< SphericalHarmonicsGravityFieldSettings >( bodySettings.at( "Sun" )->gravityFieldSettings );
                   //     double SunnominalGravitationalParameter = SunSphericalHarmonicGravityFieldSettings->getGravitationalParameter( );




        double c20Perturbation = 0.0;
        double CRPerturbation = 0.0 ;
        double shapePerturbation = 0.0 ;
        double EarthMuVariation  = 0.0 ;
        double SunMuVariation  = 0.0 ;
        ...
        if( i != 0 )
        {
            c20Perturbation = 1.0E-8 * c20PerturbationFunction( );
            CRPerturbation = 0.4 * c20PerturbationFunction( );
            shapePerturbation = 10 * c20PerturbationFunction( );
            EarthMuVariation  = 5000000000000 * c20PerturbationFunction( );
            SunMuVariation  = 5000000000000000000 * c20PerturbationFunction( );
        }

        //moonCosineSphericalHarmonicsCoefficients( 8, 8 ) += c20Perturbation;
        //moonSphericalHarmonicGravityFieldSettings->resetCosineCoefficients( moonCosineSphericalHarmonicsCoefficients );
        // radiationPressureCoefficient += CRPerturbation ;
        // referenceAreaRadiation += shapePerturbation ;
        EarthSphericalHarmonicGravityFieldSettings->resetGravitationalParameter(
                    nominalGravitationalParameter + EarthMuVariation );

        //bodySettings.at( "Sun" )->gravityFieldSettings = std::make_shared< CentralGravityFieldSettings >( nominalGravitationalParameter + SunMuVariation );
        ...
        if( generateAndCompareToBenchmark && i !=0 )
        {
            ...
            input_output::writeMatrixToFile( ( Eigen::MatrixXd( 1, 1 ) << EarthMuVariation ).finished( ),
